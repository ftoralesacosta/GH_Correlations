from default_values_iPad import *
...
def Plot_pp_pPb_Avg_FF_and_Ratio_npy():
    ...
    for SYS,sys_col in zip(reversed(Systems),reversed(Colors)):
        ...
        FF_Central = np.load("npy_files/%s_%s_Averaged_Fragmentation_Functions_%s.npy"%(Shower,SYS,description_string))
        FF_Errors = np.load("npy_files/%s_%s_Averaged_Fragmentation_Functions_Errors_%s.npy"%(Shower,SYS,description_string))
        Efficiency_Uncertainty = 0.056*FF_Central
        Sys_Uncertainty = np.load("npy_files/%s_%s_Averaged_Fragmentation_Functions_Systematics_%s.npy"%(Shower,SYS,description_string))
        ...
        if (SYS=="p-Pb"):
            leg_string = "$\mathrm{p-Pb}\ \sqrt{s_{\mathrm{_{NN}}}}=5$ TeV"
        plt.errorbar(zT_centers[:NzT-ZT_OFF_PLOT], FF_Central[:NzT-ZT_OFF_PLOT],xerr=zT_widths[:NzT-ZT_OFF_PLOT],
            yerr=FF_Errors[:NzT-ZT_OFF_PLOT],linewidth=1, fmt='o',color=sys_col,capsize=1,label=leg_string)
            #label=r' %s %1.0f < $p_\mathrm{T}^{\mathrm{trig}}$ < %1.0f GeV/$c$'%(SYS,pTbins[0],pTbins[N_pT_Bins]))
            
        Sys_Plot_pp = plt.bar(zT_centers[:NzT-ZT_OFF_PLOT], Sys_Uncertainty[:NzT-ZT_OFF_PLOT]+Sys_Uncertainty[:NzT-ZT_OFF_PLOT],
            bottom=FF_Central[:NzT-ZT_OFF_PLOT]-Sys_Uncertainty[:NzT-ZT_OFF_PLOT],width=zT_widths[:NzT-ZT_OFF_PLOT], align='center',color=sys_col,alpha=0.3)
...
#    plt.annotate("$\chi^2/\mathrm{dof}$ = %1.1f/%i, p = %1.2f"%(Chi2*NDF,NDF,Pval), xy=(0.03, 0.08), xycoords='axes fraction', ha='left', va='top', fontsize=label_size)
    ...
    pPb_Combined = np.load("npy_files/%s_p-Pb_Averaged_Fragmentation_Functions_%s.npy"%(Shower,description_string))
    pPb_Combined_Errors = np.load("npy_files/%s_p-Pb_Averaged_Fragmentation_Functions_Errors_%s.npy"%(Shower,description_string))
    pPb_Sys_Uncertainty = np.load("npy_files/%_pPb_Averaged_Fragmentation_Functions_Systematics_%s.npy"%(Shower,description_string))
    
    pp_Combined = np.load("npy_files/%s_pp_Averaged_Fragmentation_Functions_%s.npy"%(Shower,description_string))
    pp_Combined_Errors = np.load("npy_files/%s_pp_Averaged_Fragmentation_Functions_Errors_%s.npy"%(Shower,description_string))
    pp_Sys_Uncertainty = np.load("npy_files/%_pp_Averaged_Fragmentation_Functions_Systematics_%s.npy"%(Shower,description_string))
    ...
    Ratio_Systematic = np.sqrt((pp_Sys_Uncertainty/pp_Combined)**2 + (pPb_Sys_Uncertainty/pPb_Combined)**2)*Ratio
