    ...
    for SYS,ifile in zip(Systems,Files):
        ...
        for ipt in range (N_pT_Bins):
            ...
            for izt in range (NzT-ZT_OFF_PLOT):
                ...
                leg = ax.legend([s_plot,b_plot,empt,empt3,empt2],['Shower Signal Region','Background Region (scaled)',r'%1.0f < $p_\mathrm{T}^{\gamma^\mathrm{iso}}$ < %1.0f GeV/$c$'%(pTbins[ipt],pTbins[ipt+1]),r'%1.1f < $p_\mathrm{T}^\mathrm{h}$ < %1.1f GeV/$c$'%(pTbins[ipt]*zTbins[izt],hpT_Max),r'%1.2f < $z_\mathrm{T}$ < %1.2f'%(zTbins[izt],zTbins[izt+1])],
                    loc='best',title = "Alice %s 5 TeV",fontsize=26,frameon=False,numpoints=1,markerscale=2,handletextpad=0.1)
                if (SYS == 'pp'):
...
def Correlated_Subtraction_Weights(Dict):   
    for SYS in Systems:
        for ipt in range (N_pT_Bins):
            ...
            nw_BR = Dict["%s_no_weight_BR"%(SYS)]
            Cs = (Dict["%s_CSR"%(SYS)][ipt] - Dict["%s_CBR"%(SYS)][ipt])
            #Dict["%s_pUncert"%(SYS)][ipt]  = np.abs(nw_BR-Cs)*Rel_pUncert[SYS]
    ...
    for SYS,ifile in zip(Systems,Files):
        for ipt in range (N_pT_Bins):
            ...
            for izt in range (NzT):
                ...
                ZYAM_uw_BR, INCORRECT_BR_Error = ZYAM_Line(Dict["%s_no_weight_BR"%(SYS)][ipt][izt],
                                               Dict["%s_CSR_Errors"%(SYS)][ipt][izt]) #Incorrect, but we don't use the Un-weighted errors
                
                #print("%s ZYAM %i %1.4f"%(SYS,izt,ZYAM_Cs))
                old_Cs = Dict["%s_CSR"%(SYS)][ipt][izt]
                ...
                Dict["%s_no_weight_BR"%(SYS)][ipt][izt] = Dict["%s_no_weight_BR"%(SYS)][ipt][izt] - ZYAM_uw_BR
                #Dict["%s_pUncert"%(SYS)][ipt][izt] = (Dict["%s_pUncert"%(SYS)][ipt][izt]/old_Cs)*(old_Cs - ZYAM_Cs)
...
def Plot_pp_pPb_Cs_Individual(Dict):
    ...
    matplotlib.mathtext.SCRIPT_SPACE = 0.1
    for ipt in range (len(Dict["p-Pb_CSR"])):
        ...
        for izt in range (NzT-ZT_OFF_PLOT):
            ...
            if (do_sys):
                ...
                pPb_BR = Dict["%s_no_weight_BR"%("p-Pb")][ipt][izt]
                pPb_pUncert = np.abs(pPb_BR-Dict["p-Pb_CSR"][ipt][izt])*Rel_pUncert["p-Pb"]
                
                pp_BR = Dict["%s_no_weight_BR"%("pp")][ipt][izt]
                pp_pUncert = np.abs(pp_BR-Dict["pp_CSR"][ipt][izt])*Rel_pUncert["pp"]

                #Dict["%s_pUncert"%("pp")][ipt][izt] = pp_pUncert
                #Dict["%s_pUncert"%("p-Pb")][ipt][izt] = pPb_pUncert
            ...
            if (len(Dict["p-Pb_CSR"]) > 1):
                ...
                if (pTbins[N_pT_Bins]*zTbins[izt+1] > Max_Hadron_pT):
                    hpT_Max = Max_Hadron_pT
                plt.annotate(r'$%1.1f < p_\mathrm{T}^\\rm{h} < %1.1f \mathrm{GeV}/c$'%(pTbins[ipt]*zTbins[izt],hpT_Max), xy=(0.98, 0.105), xycoords='axes fraction', ha='right', va='top',fontsize=anno_size)
                ...
                if (pTbins[N_pT_Bins]*zTbins[izt+1] > Max_Hadron_pT):
                    hpT_Max = Max_Hadron_pT
                plt.annotate(r'%1.1f < $p_\mathrm{T}^\mathrm{h}$ < %1.1f GeV/$c$'%(pTbins[0]*zTbins[izt],hpT_Max), xy=(0.98, 0.105), xycoords='axes fraction', ha='right', va='top',fontsize=anno_size)
            ...
            pythia_data.values = pythia[izt][1:]
            pythia_stat = Uncertainty("pythia Statistical", is_symmetric=True)
            pythia_stat.values = pythia_error[izt][1:]
...
def Plot_pp_pPb_Cs_withPurity(Dict):
    ...
    for ipt in range (len(Dict["p-Pb_CSR"])):
        ...
        for izt in range (NzT-ZT_OFF_PLOT):
            ...
            if (izt > 2):
                izt_of7 = izt-1 
            pp_plus = np.load("npy_files/%s_%s_Combined_%s_Cs.npy"%(Shower,"pT_Rebin_1_pDevPlus","pp"))[0][izt_of7]
            pp_minus = np.load("npy_files/%s_%s_Combined_%s_Cs.npy"%(Shower,"pT_Rebin_1_pDevMinus","pp"))[0][izt_of7]
            pPb_plus = np.load("npy_files/%s_%s_Combined_%s_Cs.npy"%(Shower,"pT_Rebin_1_pDevPlus","p-Pb"))[0][izt_of7]
            pPb_minus = np.load("npy_files/%s_%s_Combined_%s_Cs.npy"%(Shower,"pT_Rebin_1_pDevMinus","p-Pb"))[0][izt_of7]

            pp_PRel_Diff = []
            pPb_PRel_Diff = []

            for iphi in range(N_dPhi_Bins):
                pp_diff = abs(pp_plus[iphi]-pp_minus[iphi])/2
                pPb_diff = abs(pPb_plus[iphi]-pPb_minus[iphi])/2
                #pp_PRel_Diff.append(pp_diff/max(abs(pp_plus[iphi]),abs(pp_minus[iphi])))
                #pPb_PRel_Diff.append(pPb_diff/max(abs(pPb_plus[iphi]),abs(pPb_minus[iphi])))

                pp_avg = abs(pp_plus[iphi] + abs(pp_minus[iphi]))/2
                pPb_avg = abs(pPb_plus[iphi] + abs(pPb_minus[iphi]))/2
                pp_PRel_Diff.append(pp_diff/pp_avg)
                pPb_PRel_Diff.append(pPb_diff/pPb_avg)

            pp_PAbs_Diff = pp_PRel_Diff*Dict["pp_CSR"][ipt][izt]
            pPb_PAbs_Diff = pPb_PRel_Diff*Dict["p-Pb_CSR"][ipt][izt]
...
#                pyth_error = plt.errorbar(delta_phi_centers,pythia[izt],pythia_error[izt],fmt="--",color="forestgreen",capsize=0)
            ...
            if (do_sys):
                ...
                Sys_Plot_pp = plt.bar(delta_phi_centers, sys_pp+sys_pp,bottom=Dict["pp_CSR"][ipt][izt]-sys_pp,width=phi_width*2,align='center',color='red',alpha=0.2,edgecolor='red')
                Sys_Plot_pPb = plt.bar(delta_phi_centers, sys_pPb+sys_pPb,bottom=Dict["p-Pb_CSR"][ipt][izt]-sys_pPb,width=phi_width*2,align='center',fill=False,edgecolor='blue')

                Purity_Compare_pp = plt.bar(delta_phi_centers, pp_PAbs_Diff+pp_PAbs_Diff,bottom=Dict["pp_CSR"][ipt][izt]-pp_PAbs_Diff,width=phi_width*2,align='center',color='orange',alpha=0.2,edgecolor='red')
                Purity_Compare_pPb = plt.bar(delta_phi_centers, pPb_PAbs_Diff+pPb_PAbs_Diff,bottom=Dict["p-Pb_CSR"][ipt][izt]-pPb_PAbs_Diff,width=phi_width*2,align='center',fill=False,edgecolor='mediumorchid',linewidth=2)
            ...
            if(Use_MC):
                leg = plt.legend([pp,pPb,Purity_Compare_pp,Purity_Compare_pPb],['pp',
                    r'p$-$Pb',r'pp $\pm \sigma_{P}$',r'p$-$Pb $\pm \sigma_{P}$'],
                    loc = "upper left",fontsize=anno_size,frameon=False,numpoints=1)
            ...
            if (len(Dict["p-Pb_CSR"]) > 1):
                ...
                if (pTbins[N_pT_Bins]*zTbins[izt+1] > Max_Hadron_pT):
                    hpT_Max = Max_Hadron_pT
                plt.annotate(r'%1.1f < $p_\mathrm{T}^\mathrm{h}$ < %1.1f GeV/$c$'%(pTbins[ipt]*zTbins[izt],hpT_Max), xy=(0.98, 0.105), xycoords='axes fraction', ha='right', va='top',fontsize=anno_size)
                plt.annotate(r'%1.0f < $p_\mathrm{T}^{\gamma}$ < %1.0f GeV/$c$'%(pTbins[ipt],pTbins[ipt+1]), xy=(0.98, 0.105), xycoords='axes fraction', ha='right', va='top', fontsize=anno_size)
            else:
                hpT_Max = pTbins[N_pT_Bins]*zTbins[izt+1]
                if (pTbins[N_pT_Bins]*zTbins[izt+1] > Max_Hadron_pT):
            ...
            plt.show()
            #fig.savefig('pics/%s/%s/Cs_Final_Indv_pT_%i_zT_%i_PurityDev_Compare.pdf'%(Shower,description_string,ipt,izt))
            fig.savefig('pics/%s/%s/GAUSSIAN_Cs_Final_Indv_pT_%i_zT_%i_PurityDev_Compare.pdf'%(Shower,description_string,ipt,izt))
...
def Integrate_Away_Side_Purity(Phi_array,Phi_Errors,UE_Error,Purity_Errors,N_Phi_Intgl=N_Phi_Integrate):
    
    Use_Uncorr_Error = True
    FF_zt = np.zeros((len(Phi_array), NzT))
    FF_zt_Errors = np.zeros((len(Phi_array), NzT))
    FF_zt_UE = np.zeros((len(Phi_array), NzT))
    FF_zt_Purity = np.zeros((len(Phi_array), NzT))
    ...
    for ipt in range(len(Phi_array)):
        ...
        for izt in range(0, NzT):
            ...
            #temp_error = (Phi_Errors[ipt][izt][-N_Phi_Intgl:]/(dPhi_Width*N_Phi_Intgl))**2
            temp_error = (Phi_Errors[ipt][izt][-N_Phi_Intgl:]/(N_Phi_Intgl))**2
            temp_pur = (Purity_Errors[ipt][izt][-N_Phi_Intgl:]/(N_Phi_Intgl))**2 #Bullshit, the purity is 100% correlated between zT points
            
            if (Use_Uncorr_Error):
                FF_zt_Purity[ipt][izt] = math.sqrt(temp_pur.sum())/zT_width
                FF_zt_Errors[ipt][izt] = (math.sqrt(temp_error.sum() + (UE_Error[ipt][izt][0])**2))/zT_width
                FF_zt_UE[ipt][izt] = UE_Error[ipt][izt][0]/zT_width
    ...
    return FF_zt, FF_zt_Errors,FF_zt_UE,FF_zt_Purity
...
def Get_Fragmentation(Dict,N_Phi_Intgl=N_Phi_Integrate,Use_Avg_Cs=False):
    ...
    for index,SYS in enumerate(Systems):
        ...
        #IRC Prop. Uncomment above when going back to relative errors
        #temp_FF, temp_FF_Errors, temp_FF_UE_Errors,temp_purity_Errors = Integrate_Away_Side_Purity(Dict["%s_CSR"%(SYS)],Dict["%s_CSR_Errors"%(SYS)],Dict["%s_Uncorr_Error"%(SYS)],Dict["%s_pUncert"%(SYS)],N_Phi_Intgl,)    
        
        if (Use_Avg_Cs):
            temp_FF,temp_FF_Errors,temp_purity_Errors, temp_FF_UE_Errors = temp_FF[0],temp_FF_Errors[0],temp_purity_Errors[0] #first element because already averaged and flattened array
...
def Plot_pp_pPb_Cs_Individual_Extra_zT(Dict):
    ...
    for ipt in range (len(Dict["p-Pb_CSR"])):
        ...
        for izt in range (NzT-ZT_OFF_PLOT):
            if ((izt == 0) or (izt == 3) or (izt == 7)):
                continue
            #fig = plt.figure(figsize=(10,12))

            ax = fig.add_subplot(2,3,izt+1)

            pPb_error = ax.errorbar(delta_phi_centers,Dict["p-Pb_CSR"][ipt][izt],xerr=phi_width*0,yerr=Dict["p-Pb_CSR_Errors"][ipt][izt],fmt='bo',capsize=0,markersize=11)
            pp_error = ax.errorbar(delta_phi_centers,Dict["pp_CSR"][ipt][izt],xerr=phi_width*0,yerr=Dict["pp_CSR_Errors"][ipt][izt],fmt='rs',capsize=0,markersize=11)
            pPb = ax.errorbar(delta_phi_centers,Dict["p-Pb_CSR"][ipt][izt],xerr=phi_width,yerr=Dict["p-Pb_CSR_Errors"][ipt][izt],fmt='bo',lw=0,capsize=0,markersize=11)
            pp = ax.errorbar(delta_phi_centers,Dict["pp_CSR"][ipt][izt],xerr=phi_width,yerr=Dict["pp_CSR_Errors"][ipt][izt],fmt='rs',lw=0,capsize=0,markersize=11)

            if (Use_MC):
                pyth = ax.plot(delta_phi_centers,pythia[izt],"--",color="forestgreen",label='PYTHIA 8.2 Monash')
                pyth_error = ax.errorbar(delta_phi_centers,pythia[izt],pythia_error[izt],fmt="--",color="forestgreen",capsize=0)
            ...
            plt.show()
        fig.savefig('pics/%s/%s/Cs_Final_Indv_pT_%i_zT_%i_Extra_zT.pdf'%(Shower,description_string,ipt,99))
