...
def GetPhiProj(filename,prfx,ipt, izt, Signal_DNN=True,Unweight=False):
    ...
    if (Signal_DNN == "Inclusive"):
        ...
        if (not(Signal_DNN) and (Unweight) ):
...
def ROOT_to_nparray():
    ...
    for SYS,ifile in zip(Systems,Files):
        ...
        Keys.append("%s_no_weight_BR"%(SYS))
    ...
    for SYS,ifile in zip(Systems,Files):    
        for ipt in range (N_pT_Bins):
            ...
            for izt in range (0,NzT):
                ...
                Sig_Phi_Array, Sig_Phi_Error_Array = GetPhiProj(ifile,SYS,ipt,izt,True)
                Bkg_Phi_Array, Bkg_Phi_Error_Array = GetPhiProj(ifile,SYS,ipt,izt,False)
                no_weight_BR, no_weight_BR_error = GetPhiProj(ifile,SYS,ipt,izt,False,True)
                ...
                if (Average_UE):
                    ...
                    csr = Sig_Phi_Array/dPhi_Width
                    cbr = Bkg_Phi_Array/dPhi_Width
                    nw_cbr = no_weight_BR/dPhi_Width
                    Dict["%s_pUncert"%(SYS)][ipt][izt] = np.abs((csr-cbr)-nw_cbr)*purity_Uncertainty[SYS]/(purity[SYS])
                    Dict["%s_no_weight_BR"%(SYS)][ipt][izt] = no_weight_BR/dPhi_Width
