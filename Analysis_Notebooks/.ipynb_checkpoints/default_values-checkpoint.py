...
#description_string = "zT_Rebin_8_006zT06zTITSSub"
...
if (N_dPhi_Bins == 8):
    ...
    for ipyth,ipyth_error,i in zip(pythia,pythia_error,range(len(pythia))):
        ipyth = np.asarray(ipyth)
        ipyth_error = np.asarray(ipyth_error)
        ipyth = map(sum, zip(ipyth[::2], ipyth[1::2]))
        ipyth_error = map(sum, zip(ipyth_error[::2], ipyth_error[1::2]))
        pythia[i] = ipyth
        pythia_error[i] = ipyth_error
