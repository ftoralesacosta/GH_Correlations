  ...
  //LOOP OVER SAMPLES
   ...
   int skip_counter = 0;
    for(Long64_t ievent = 0; ievent < nentries ; ievent++){     
      ...
      for (ULong64_t itk = 0; itk < ntrack; itk++) {
	if(track_pt[itk] < 0.5) continue; //500 MeV Tracks                                                                                                             
	if(track_pt[itk] > 30) continue;
	if((track_quality[itk]&Track_Cut_Bit)==0) continue; //select only tracks that pass selection                                                                   
	if(abs(track_eta[itk]) > 0.8) continue;
	if( not(track_its_ncluster[itk]>4)) continue;
	if( not(track_its_chi_square[itk]/track_its_ncluster[itk] <36)) continue;
	if( not(TMath::Abs(track_dca_xy[itk])<0.0231+0.0315/TMath::Power(track_pt[itk],1.3 ))) continue;
	...
	for (Long64_t imix = mix_start; imix < mix_end+1; imix++){
	  ...
	  if (first_track) ME_pass_Counter ++;  //simple conuter for ME that pass
	  ...
	  for (ULong64_t itrack = 0; itrack < ntrack_max; itrack++) {
	    ...
	    //Observables
 	    Double_t zt = track_data_out[0][itrack][1]/track_pt[itk];
	    Float_t DeltaPhi = TMath::Abs(TVector2::Phi_mpi_pi(track_phi[itk] - track_data_out[0][itrack][3]));
	    Float_t DeltaEta = track_eta[itk] - track_data_out[0][itrack][2];
	    if ((TMath::Abs(DeltaPhi) < 0.005) && (TMath::Abs(DeltaEta) < 0.005)) continue;

	    All_Dihadron_Corr->Fill(DeltaPhi,DeltaEta);

	    for (int ipt = 0; ipt < n_track_ptbins; ipt++){
	      if (track_pt[itk] > track_ptbins[ipt] && track_pt[itk] < track_ptbins[ipt+1]){
		for (int izt =0; izt < nztbins; izt++){
