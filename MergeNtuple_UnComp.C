                   ...
                   const char *glob)
{
    ...
    const TList *key_list_1 = test_file->GetListOfKeys();
        ...
        const TList *key_list_2 = test_file->GetListOfKeys();

        for (Int_t j = 0; j < key_list_2->GetEntries(); j++) {
	  const TString n = key_list_2->At(j)->GetName();
                  ...
                  TObjArray &dir_tree_name, const char *glob)
{
    ...
    TFile output_file(output_filename, "RECREATE","uncomp",0);

    for (Int_t i = 0; i < dir_tree_name.GetEntries(); i++) {
        output_file.cd();

        TString* dn = new TString("AliAnalysisTaskNTGJ");
	const TString *dtn = (TString *)dir_tree_name.At(i);
        ...
        ((TChain *)chain[i])->Merge(&output_file, 0, "keepC");
