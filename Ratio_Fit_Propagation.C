...
void Ratio_Fit_Propagation(){
  ...
  TGraphErrors *prop = new TGraphErrors(NzT);
  prop->SetTitle("Propagated Systematic Unceratainty Band");

  for (int i=0; i<100; i++){
    grint->SetPoint(i, (0.66/100)*i,0);
    prop->SetPoint(i,(0.66/100)*i,0.84);
    prop->SetPointError(i,0,0.19);
  }
  ...
  prop->SetLineColorAlpha(kBlue,1);
  prop->SetFillColorAlpha(kBlue,0.2);
  prop->Draw("Samee3");
